...
Funciones comunes para la tanda de 13 características.
...
Formato esperado:
    dato_expandido_con_entropia.csv -> n_muestras*20 filas x 13 columnas
    salida_expandido.csv            -> n_muestras filas x 3 columnas
...
def load_20x13_data():
    ...
    if X_raw.shape[1] != N_FEATURES:
        raise ValueError(
            f"{X_FILENAME} debe tener {N_FEATURES} columnas. Tiene {X_raw.shape[1]}."
        )

    if Y.shape[1] != N_CLASSES:
        raise ValueError(
            f"{Y_FILENAME} debe tener {N_CLASSES} columnas. Tiene {Y.shape[1]}."
        )
