...
# -*- coding: utf-8 -*-
"""
utils_13_features_20x13.py

Funciones comunes para trabajar con 13 características.

Archivos esperados:
    dato_expandido_con_entropia.csv
    salida_expandido.csv

Formato esperado:
X_FILENAME = "dato_expandido_con_entropia_ruido.csv" -> n_muestras*20 filas x 13 columnas
Y_FILENAME = "salida_expandido.csv"-> n_muestras filas x 3 columnas

Reconstrucción:
    X -> n_muestras x 20 pasos x 13 variables

Notas:
    - El escalado se ajusta solamente con entrenamiento.
    - El split se realiza por muestra/secuencia, no por fila individual.
    - Para modelos clásicos se puede aplanar cada muestra como 20 x 13 = 260 variables.
"""
...
def _resolve_file(filename):
    """
    Busca el archivo en:
    1) carpeta actual desde donde se ejecuta Python;
    2) carpeta padre;
    3) carpeta donde está este archivo utils.
    Así evitamos el circo de rutas relativas, porque bastante tenemos.
    """
    candidates = [
        Path.cwd() / filename,
...
def load_20x13_data():
    ...
    if X_raw.shape[1] != N_FEATURES:
        raise ValueError(
            f"{X_FILENAME} debe tener {N_FEATURES} columnas. "
            f"Tiene {X_raw.shape[1]}."
        )

    if Y.shape[1] != N_CLASSES:
        raise ValueError(
            f"{Y_FILENAME} debe tener {N_CLASSES} columnas. "
            f"Tiene {Y.shape[1]}."
        )
...
def evaluate_and_export(model_name, out_prefix, y_test, y_prob, history=None, extra_config=None):
    ...
    if history is not None:
        pd.DataFrame(history.history).to_csv(f"history_{out_prefix}.csv", index=False)

        fig, ax = plt.subplots(1, 2, figsize=(12, 4))

        ax[0].plot(history.history.get("accuracy", []), label="Entrenamiento")
        ax[0].plot(history.history.get("val_accuracy", []), label="Validación")
        ...
        ax[1].plot(history.history.get("loss", []), label="Entrenamiento")
        ax[1].plot(history.history.get("val_loss", []), label="Validación")
