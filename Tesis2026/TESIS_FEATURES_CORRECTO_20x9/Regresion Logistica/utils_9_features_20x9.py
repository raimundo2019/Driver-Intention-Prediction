...
Funciones comunes para la tanda definitiva de 9 caracteristicas.
...
Cada script se ejecuta desde su propia carpeta y lee:
    ../dato_9.csv
    ../salida_9.csv

Formato:
    dato_9.csv   -> 60000 filas x 9 columnas
    salida_9.csv -> 3000 filas x 3 columnas

Reconstruccion:
    X -> 3000 muestras x 20 pasos x 9 variables
"""
...
def load_20x9_data():
    ...
    if X_raw.shape[1] != N_FEATURES:
        raise ValueError(f"dato_9.csv debe tener 9 columnas. Tiene {X_raw.shape[1]}.")

    if Y.shape[1] != N_CLASSES:
        raise ValueError(f"salida_9.csv debe tener 3 columnas. Tiene {Y.shape[1]}.")
    ...
    if X_raw.shape[0] != expected_rows:
        raise ValueError(
            f"Cantidad de filas incompatible. dato_9.csv tiene {X_raw.shape[0]} filas, "
...
def evaluate_and_export(model_name, out_prefix, y_test, y_prob, history=None, extra_config=None):
    ...
    if history is not None:
        ...
        ax[0].plot(history.history.get("val_accuracy", []), label="Validacion")
        ...
        ax[1].plot(history.history.get("val_loss", []), label="Validacion")
