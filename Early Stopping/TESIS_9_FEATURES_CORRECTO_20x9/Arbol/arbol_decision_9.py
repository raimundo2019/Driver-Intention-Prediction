...
# -*- coding: utf-8 -*-
"""
arbol_decision_9_earlystopping_corregido.py

Arbol de Decision para 9 caracteristicas.

Correcciones incorporadas:
1) Uso explicito de conjunto de validacion.
2) Uso de etiquetas enteras para log_loss: y_train_lbl, y_val_lbl.
3) Criterio tipo Early Stopping sobre val_loss.
4) Grafico Training Accuracy + Validation Accuracy.
5) Grafico Training Loss + Validation Loss.

IMPORTANTE:
    Un arbol de decision de scikit-learn no entrena por epocas reales.
    Para visualizar curvas tipo entrenamiento, se entrena una secuencia de
    arboles aumentando max_depth desde 1 hasta EPOCHS.
"""
...
def guardar_curvas_entrenamiento(history, out_prefix, epochs_configuradas, patience):
    ...
    ax[0].plot(epochs_axis, history["accuracy"], label="Training accuracy", linewidth=2)
    ...
    ax[1].plot(epochs_axis, history["loss"], label="Training loss", linewidth=2)
    ...
    fig.suptitle(
        f"Arbol de Decision 9 caracteristicas - Early Stopping "
...
# CORRECCION CLAVE: log_loss necesita etiquetas 0,1,2, no one-hot.
y_train_lbl = y_train.argmax(axis=1)
y_val_lbl = y_val.argmax(axis=1)
y_test_lbl = y_test.argmax(axis=1)

history = {"epoch": [], "max_depth": [], "accuracy": [], "val_accuracy": [], "loss": [], "val_loss": []}
...
for depth in range(1, EPOCHS + 1):
    model_stage = DecisionTreeClassifier(random_state=RANDOM_STATE, max_depth=depth)
    model_stage.fit(X_train_f, y_train_lbl)

    y_train_pred = model_stage.predict(X_train_f)
    y_val_pred = model_stage.predict(X_val_f)
    y_train_prob = align_proba(model_stage.predict_proba(X_train_f), model_stage.classes_, N_CLASSES)
    y_val_prob = align_proba(model_stage.predict_proba(X_val_f), model_stage.classes_, N_CLASSES)
    ...
    print(
        f"Etapa {depth:03d}/{EPOCHS} | max_depth={depth:03d} | "
...
if best_model is None:
    raise RuntimeError("No se encontro ningun mejor modelo. Revisar EPOCHS, PATIENCE o los datos.")

model = best_model
y_prob = align_proba(model.predict_proba(X_test_f), model.classes_, N_CLASSES)

joblib.dump(model, f"modelo_{OUT_PREFIX}_earlystopping.pkl")
joblib.dump(scaler, f"scaler_{OUT_PREFIX}_earlystopping.pkl")

evaluate_and_export(
    "Arbol de Decision 9 caracteristicas con Early Stopping",
...
feature_names = [f"x{j+1}_t{i+1:02d}" for i in range(TIMESTEPS) for j in range(N_FEATURES)]
...
plt.title(f"Arbol de decision - 9 caracteristicas - best max_depth={best_depth}")
plt.tight_layout()
