...
# -*- coding: utf-8 -*-
"""
cnn_rnn_13_earlystopping_train_val.py

Modelo hibrido CNN-RNN para 13 caracteristicas con entrada temporal real 20 x 13.
...
OUT_PREFIX = "cnn_rnn_13_mejorado"
...
def guardar_grafico_entrenamiento(history, out_prefix, epochs_configuradas, patience):
    ...
    fig.suptitle(
        f"CNN-RNN 13 caracteristicas - Early Stopping "
...
evaluate_and_export(
    "CNN-RNN mejorado 13 caracteristicas con Early Stopping",
    OUT_PREFIX,
    y_test,
    y_prob,
    history=history,
    extra_config={
        "arquitectura": (
            "Conv1D(100) + BatchNormalization + Dropout(0.15) + "
            "Conv1D(64) + BatchNormalization + MaxPooling1D + Dropout(0.15) + "
            "SimpleRNN(110) + Dense(100) + Dense(50) + softmax"
        ),
        "entrada_modelo": "20 pasos x 13 variables",
        "tipo_modelo": "Hibrido CNN-RNN",
